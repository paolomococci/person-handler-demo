...
class PersonRestControllerMockMvcTests {
    ...
    fun `create test`() {
        mockMvc!!.perform(post("/api/persons")
                .content(paul).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isCreated)
                .andExpect(header().string("Location", containsString("api/persons/")))
    }

    @Test
    @Throws(Exception::class)
    fun `retrieve test`() {
        val mvcResult = mockMvc!!.perform(post("/api/persons")
                .content(paul).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isCreated)
                .andReturn()
    ...
    fun `update test`() {
        ...
        mockMvc.perform(put(result!!)
                .content("{\"name\":\"Jessy\",\"surname\":\"Smith\",\"age\":\"25\"}")
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isCreated)
        mockMvc.perform(get(result))
                .andExpect(status().isOk)
                .andExpect(jsonPath("$.name").value("Jessy"))
                .andExpect(jsonPath("$.surname").value("Smith"))
    ...
    fun `partial update test`() {
        val mvcResult = mockMvc!!.perform(post("/api/persons")
                ...
                .andReturn()
        val result = mvcResult.response.getHeader("Location")
        mockMvc.perform(patch(result!!)
                .content("{\"age\":\"36\"}").contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isCreated)
        mockMvc.perform(get(result))
                .andExpect(status().isOk)
    ...
    fun `verify find by name`() {
        mockMvc!!.perform(post("/api/persons")
                .content(paul).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isCreated)
        mockMvc.perform(get("/api/persons/name/{name}", "Paul"))
                .andExpect(status().isOk)
    ...
    fun `verify find by surname`() {
        mockMvc!!.perform(post("/api/persons")
                .content(paul).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isCreated)
        mockMvc.perform(get("/api/persons/surname/{surname}", "Smith"))
                .andExpect(status().isOk)
    ...
    fun `verify find by age`() {
        mockMvc!!.perform(post("/api/persons")
                .content(paul).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isCreated)
        mockMvc.perform(get("/api/persons/age/{age}", 35))
                .andExpect(status().isOk)
                .andExpect(jsonPath("$._embedded.persons[0].name").value("Paul"))
    }
}
